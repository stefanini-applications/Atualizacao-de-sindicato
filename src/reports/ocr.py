...
def imprimir_relatorio_ocr(processados: List[TextoExtraido]) -> None:
    """Imprime resumo do OCR com os três contadores de status (AC6)."""
    total = len(processados)

    contadores = {s: 0 for s in _STATUS_OCR_ORDENADOS}
    for t in processados:
    ...
    print()
    for status in _STATUS_OCR_ORDENADOS:
        label = _LABELS_OCR.get(status, status)
